<!DOCTYPE CW><CW>
<customwidgets>
    <customwidget>
        <class>Preview</class>
        <header location="global">F:/Van Dalen/Mijn documenten/Rogier/Fonts/TrueTypeViewer/preview.h</header>
        <sizehint>
            <width>50</width>
            <height>20</height>
        </sizehint>
        <container>0</container>
        <sizepolicy>
            <hordata>7</hordata>
            <verdata>7</verdata>
        </sizepolicy>
        <pixmap>
            <data format="XPM.GZ" length="646"></data>
        </pixmap>
        <slot access="public">setText(const QString &amp;)</slot>
        <slot access="public">setFontCache(FontCache *)</slot>
        <slot access="public">setSubPixel(bool)</slot>
    </customwidget>
    <customwidget>
        <class>GlyphView</class>
        <header location="local">glyphview.h</header>
        <sizehint>
            <width>48</width>
            <height>48</height>
        </sizehint>
        <container>0</container>
        <sizepolicy>
            <hordata>7</hordata>
            <verdata>7</verdata>
        </sizepolicy>
        <pixmap>
            <data format="XPM.GZ" length="646"></data>
        </pixmap>
    </customwidget>
</customwidgets>
</CW>
